...
if (($rec_id > 0) && ($rec_nom != "") && ($rec_description != "") && ($rec_fk_categorie_id != ""))
{
...
//         // DONNER LE MESSAGE A AFFICHER EN FEEDBACK
    ...
    $tabAssoColVal = [
        ...
        // "re_fk_us_id"           => $user_id,
            ...
            // SI ON A UPLOADE UN NOUVEAU FICHIER,  ALORS ON MET A JOUR LA VALEUR DANS SQL.  SINON, ON LAISSE LA VALEUR ACTUELLE
            if ($imageSrc != "")
            {
                $tabAssoColVal["imageSrc"] = $imageSrc;
            }
 
    
    // DONNER LE MESSAGE A AFFICHER EN FEEDBACK
    $recetteFeedback = "Votre recette a bien été modifiée.";
    ...
    if($ing_nom_count < 1 ) {
        ...
        $objetPDOStatement = modifierLigneSQL("rp_recette", $tabAssoColVal, $rec_id);
        $objetPDOStatement = modifierLigneSQL("rp_ingredient", $tabAssoColVal1, $ing_id);
        $objetPDOStatement = modifierLigneSQL("rp_etape", $tabAssoColVal2, $eta_id);
        
        // DONNER LE MESSAGE A AFFICHER EN FEEDBACK
        $articleFeedback =
<<<CODEHTML
        <p>VOTRE ARTICLE EST MODIFIE</p>
        <div><a href="template-article.php?use_id=$use_id">CLIQUER ICI POUR VOIR VOTRE ARTICLE</a></div>
